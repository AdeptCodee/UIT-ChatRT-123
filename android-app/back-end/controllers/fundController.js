...
export const createFund = async (req, res) => {
  try {
    ...
    if (!conversationId || !title || !totalAmount || !totalDays) {
      return res.status(400).json({ message: "Thiếu thông tin: Cần có conversationId, title, totalAmount và totalDays." });
    }
    ...
    if (memberIds.length === 0) {
      return res.status(400).json({ message: "Nhóm chat không có thành viên nào hợp lệ." });
    }
    ...
    if (isNaN(amount) || isNaN(days) || days <= 0) {
      return res.status(400).json({ message: "Số tiền hoặc số ngày không hợp lệ." });
    }
...
export const skipDay = async (req, res) => {
  try {
    ...
    if (!fund) return res.status(404).json({ message: "Không có quỹ nào đang chạy trong nhóm này." });
    ...
    let reminderContent = `📢 [NHẮC ĐÓNG QUỸ - NGÀY ${fund.currentDay}]\n` +
                          `📌 Quỹ: ${fund.title}\n` +
    ...
    res.status(200).json({ message: `Đã chuyển sang Ngày ${fund.currentDay}`, currentDay: fund.currentDay });
  } catch (error) {
